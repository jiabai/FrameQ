...
| `auth.ts` / `adminAuth.ts` / `prismaStore.ts` | 当前 OTP 验证由多次 Store 调用完成，ticket 先消费再创建 session；额度 transaction 仍先读 `used < limit` 再无条件 increment。现有行为存在并发 check-then-write 窗口，已形成持久设计和两个 active ExecPlan，但尚未实现 | 如何按 `docs/design-docs/2026-07-22-server-auth-quota-operations-hardening.md` 把 purpose-scoped OTP、artifact、ticket/session 与 quota/event 收口成语义 Store 事务，并用独立 Prisma clients 证明不重复、不超额和故障回滚？ |
| `store.ts` / `prismaStore.ts` | broad Store port；MemoryStore 与 records 同文件；PrismaStore 镜像所有方法；认证/额度加固已决定优先增加 purpose-specific semantic operations，而不是把 transaction callback 暴露给 route/service | 完成安全事务边界后，是否仍有可独立演进且不跨实体提交的子域值得拆 port；拆分怎样避免破坏语义 transaction facade？ |
