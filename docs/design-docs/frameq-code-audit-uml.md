...
| `app/src-tauri/src/worker_runtime/runner.rs` | 2,162 | 四类 worker 操作的唯一生命周期编排；生产区约 1,006 行，另含约 1,156 行内联测试；私有职责拆分设计已批准但尚未实施 |
...
| `app/src-tauri/src/worker_runtime/runner.rs` | 当前 2,162 行，约 1,006 行生产区同时编排 process I/O、watchdog、progress 与 terminal policy，另有约 1,156 行内联测试；`WorkerLane` 仍是正确的唯一生命周期入口 | 按已批准设计提取四个私有 owner，并保持 root-only 编排、现有类型路径、完整生命周期顺序、matching-instance cleanup、structured-result-first precedence、supervisor-only signalling 与安全日志；不得新增第二个 executor |
...
| Worker lifecycle runner and progress routes | stable `app/src-tauri/src/worker_runtime/runner.rs`; approved future private owners `runner/process_io.rs`, `progress.rs`, `terminal.rs`, `watchdog.rs` |
