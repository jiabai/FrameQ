...
5. Python worker 解析来源，经媒体准备门面下载并校验视频、准备任务内 video/audio 和字幕候选；pipeline 有可用字幕时直接写出，否则使用已准备的音频运行 ASR。
...
| `server/src/server.ts` | 112 | 稳定 composition root：Fastify、六个 service、运行配置、release manifest、全局 raw-JSON parser 与 registrar 组合 |
| `server/src/routes/*.ts` | 764 | 六个 capability HTTP adapter 加私有 auth schema/shared helpers；最大单文件 `admin.ts` 为 330 行 |
...
auth schema 和窄 helper；其中最大 `admin.ts` 为 330 行。总行数略增来自显式依赖与边界测试，
审计价值是 root 生命周期、HTTP capability、安全 policy 和 Store 事务各有单一 owner，而不是
把路由复杂度隐藏到新的 plugin 或 facade。
...
flowchart LR
  ...
  subgraph AccountService["FrameQ account service"]
    Fastify["server.ts<br/>composition root"]
...
- `pipeline.py` 只编排阶段；URL 下载、媒体校验、任务内视频/音频准备和字幕发现统一经
  `MediaPreparationFacade.prepare()` 返回 `PreparedMedia`。
...
- `worker_service.py` 与 `pipeline.py` 互相分担 application orchestration；媒体准备已下沉到
  `MediaPreparationFacade`，`cli.py` 仍保留若干 pipeline/media helper 兼容入口。
- `server.ts` 只保留稳定 composition root；`routes/*` 按 capability 拥有全部 HTTP adapter，
  并由 boundary tests 禁止 feature-to-feature、Prisma 和 plugin import。
...
**怎么读：** 从 `CLI -> WorkerService -> Pipeline` 看主调用链，再分别沿
MediaPreparationFacade、Transcriber、InsightFlow 和 TaskStoreFacade 四个方向展开。Protocol
的虚线实现关系表示已有可替换边界。
...
classDiagram
  ...
  Pipeline --> ProcessRequest : receives
  Pipeline --> UrlMediaSource : constructs
  Pipeline --> MediaPreparationFacade
  Pipeline --> PreparedMedia : consumes
...
3. `pipeline.py` 负责完整视频任务的阶段顺序；它创建 `UrlMediaSource` 并消费
   `PreparedMedia`，但不再直接下载、选择、探测媒体或提取音频。AI retry 只复用其中的 AI generation step，不会重新下载或转写。
...
- `pipeline.py` 包含顶层编排、subtitle/ASR/AI stage helper 和路径 resolver；下载结果选择、媒体
  探测、音频复用/提取与字幕发现 helper 已移入 `media_preparation.py`。
...
- `test_media_preparation.py` 以行为和 AST gate 证明 pipeline 不再绕过 facade，facade 也不吸收
  ASR、AI 或 task persistence 所有权。
...
**怎么读：** `Index` 是进程启动点，`FastifyServer` 创建 services、安装 root parser，并同步组合
私有 `RouteRegistrars`。registrar 只负责 capability HTTP adapter；所有 service 都依赖抽象
`Store`，生产环境使用 `PrismaStore`，测试可使用 `MemoryStore`。
...
`server.ts` 已从 710 行收口为 112 行，只负责 composition 生命周期；六个私有 route registrar
按 capability 拥有 HTTP schema、认证和 response mapping。拆分没有移动 `Store`/`PrismaStore`
事务：本应原子的读取和写入仍不会散落到多个 service/route 调用中。

### 服务端所有权说明

- `buildServer()` 创建 service、root parser 和运行配置，再同步调用私有 registrar；它不再拥有
  直接 handler 或 Zod schema。
- 六个 registrar 分别拥有 admin、desktop auth/account/LLM/update 和 billing HTTP adapter，
...
| `server.ts` | 历史 710 行热点已收口为 112 行稳定 composition root；私有 registrar 分别拥有 admin、desktop auth/account/LLM/update、billing HTTP adapter，完整证据见上方“已解决审计项” | 后续 route 变化应进入对应 capability owner，并保持 root-only Fastify/service/parser composition、admin cookie/CSRF、billing raw-body 和 Store/Prisma transaction 门禁；不要继续机械增加 facade 或改成 plugin 层级 |
...
| Fastify composition root | `server/src/server.ts` |
| Capability HTTP adapters | `server/src/routes/admin.ts`, `desktopAuth.ts`, `desktopAccount.ts`, `desktopLlm.ts`, `desktopUpdates.ts`, `billing.ts`, `authSchemas.ts`, `shared.ts` |
