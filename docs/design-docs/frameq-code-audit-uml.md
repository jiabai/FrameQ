...
1. `cli.py` 当前负责进程协议，也是 source resolution 的生产 composition root：识别固定 mode、读取有上限的 stdin、构建默认平台 resolver，并把它注入 application service。已接受但尚未实施的 `docs/design-docs/2026-07-24-python-worker-application-facade.md` 会把生产默认组合移入 application 层，并将 CLI 收窄为纯进程 adapter；TDD 实施步骤已登记在 `docs/exec-plans/active/2026-07-24-python-worker-application-facade-plan.md`。
...
| `worker/frameq_worker/worker_service.py` | 仍是五类 worker use case 的 application facade，负责 URL、本地媒体、source identity、AI retry 与 model download composition；五 handler 拆分设计与 TDD ExecPlan 已接受但尚未实施 | 按 `docs/design-docs/2026-07-24-python-worker-application-facade.md` 与 `docs/exec-plans/active/2026-07-24-python-worker-application-facade-plan.md` 保留五函数稳定 facade、唯一 retry task-persistence owner、稳定 stdin/result contract 与 root-only handler imports |
...
| `worker/frameq_worker/cli.py` | 同时承担进程 adapter、source resolver composition root；40-name `__all__` 还重导出 pipeline、request、ASR 和 helper symbols，四个 forwarding wrapper 使用 `*args/**kwargs` | 已确认兼容导出只有仓库测试依赖；按已登记的设计与 active ExecPlan 迁移测试到真实 owner，并把 CLI 收窄为 mode/stdin/render/dispatch |
