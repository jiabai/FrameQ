...
| 小红书与抖音 fallback | 仍各自包含 HTTP response、headers、decode、候选排序和下载 helper，已有部分 `download_reliability.py` 共用能力 | 完成 Bilibili 独立验证后分别重新审计；不得不经平台差异分析直接复制 Bilibili 的包结构 |
