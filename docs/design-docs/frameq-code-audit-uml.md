...
| WorkerLane | Rust application module 可调用的受监督进程通道，统一提供 `run`、`cancel` 和 `is_active`；内部组合 `ProcessSupervisor` |
...
| 项目 | 基线 |
|------|------|
| 基准提交 | `cfd1233b5091004fea4ab736b3f1801a8e88216a` |
| 提交日期 | 2026-07-18 |
| 提交主题 | `refactor(contract): enforce process-video request v3` |
...
| Tauri / Rust | 22 | 11,392 |
| Python worker | 30 | 9,268 |
...
| 文件 | 物理行数 | 当前可见职责 |
|------|---------:|--------------|
| `app/src-tauri/src/video_processing.rs` | 1,327 | IPC/worker DTO、配置解析、缓存查找、source identity 预检、lane 调用、结果映射、AI retry |
...
flowchart LR
  subgraph DesktopProcess["Desktop process"]
    ...
    subgraph TauriLayer["Tauri / Rust"]
      ...
      WorkerRuntime["worker_runtime/<br/>command · runner · supervisor"]
...
**怎么读：** 从 `LibRegistry` 看 command 如何注册，再沿 `VideoProcessingModule -> WorkerInvocation -> WorkerCommandSpec` 看子进程如何被描述；application module 只调用 `WorkerLane`，lane 在内部组合私有 `ProcessSupervisor` 并拥有完整运行顺序。`TaskManifestModule` 一组展示的是独立的本地存储信任边界。
...
classDiagram
  ...
  VideoProcessingModule --> WorkerRunRequest
  ...
  WorkerLane --> WorkerRunRequest : accepts
...
| 受监督 worker runtime | `worker_runtime/command.rs`, `runner.rs`, `supervisor.rs` | 分别拥有固定 command/env、完整 child 生命周期/进度/terminal 分类、私有取消状态与平台信号 |
...
继续审计时应确认其他代码不能绕过 storage trust boundary 或 `WorkerLane`，而不是重新把
低层生命周期 helper 暴露为 crate-wide 公共 API。
...
用 instance ID 防止旧 waiter 清除新进程。application module 看见的是 `WorkerLane`：video 和
model download 各自拥有一个 lane，因此可以独立监督，但同一 lane 不允许同时启动两个
child。

### Rust 所有权说明

- `ProcessSupervisors` 组合两个 `WorkerLane`；每个 lane 私有组合一个 `ProcessSupervisor`。
- `video_processing.rs` 既是 IPC adapter，也是 video lane application orchestrator，但只能通过
  lane 的 `run/cancel/is_active` policy-shaped API 操作 child 生命周期。
- `worker_runtime/command.rs` 拥有固定调用、环境和 bounded stdin；`runner.rs` 是四类操作唯一的
  ...
  Windows/macOS 进程树终止。
- `ProgressRoute` 是 `None | Worker | AsrModelDownload` 的闭集，application module 不能注入任意
  parser、事件名或未验证 payload。
...
sequenceDiagram
  ...
  participant Runtime as WorkerLane / worker_runtime
  ...
  alt Direct request cache hit
    ...
    Tauri->>Runtime: run ResolveSourceIdentity request
    ...
    alt Identity cache hit
      ...
      Tauri->>Runtime: run ProcessVideo request
...
4. **Rust 启动正式 worker。** Rust 已把 UI 意图解析成 v3
   `contract_version + url + asr_model` 请求；`WorkerLane` 把它放在 bounded stdin，而不是
...
2. `cancel_process` 调用 video `WorkerLane::cancel()`；lane 内部的 `ProcessSupervisor` claim
   cancellation。
...
sequenceDiagram
  ...
  Tauri->>Runtime: run RetryInsights with bounded stdin and managed env
...
| `video_processing.rs` | IPC/worker DTO、ASR 配置解析、两次 cache policy、identity preflight、lane 调用、AI retry 和 public result mapping 位于一文件；raw child 生命周期已下沉 | cache/preflight/request preparation/result mapping 中哪些可成为独立 application service；如何保持 model-aware cache、typed lane outcome 和 safe logging？ |
