...
| ProcessSupervisor | Rust 中记录受控子进程实例、阶段和取消竞争的状态机，不是 Python worker 本身 |
...
| 项目 | 基线 |
|------|------|
| 基准提交 | `f22861cab9ef1790ffa3090cc643a8ab684d41d4` |
| 提交日期 | 2026-07-18 |
| 提交主题 | `refactor(worker): invert source identity dependencies` |
...
| 跨进程契约 | `contracts/desktop-worker-contract.json`，当前实现版本 v2 |

范围规则：

- 图只表示该提交中的生产实现，不把 active ExecPlan 当作已落地代码。
- `process_local_media` 尚未注册为 Tauri command，也不在 worker 当前请求模型中，因此本基线不画入本地媒体导入流程。
...
| 子系统 | 生产源文件数 | 物理行数 |
|--------|-------------:|---------:|
| React / TypeScript | 76 | 13,486 |
| Tauri / Rust | 19 | 10,902 |
| Python worker | 30 | 9,292 |
...
| 文件 | 物理行数 | 当前可见职责 |
|------|---------:|--------------|
| `app/src-tauri/src/video_processing.rs` | 1,491 | IPC DTO、缓存查找、source identity 预检、worker 编排、进度转发、结果映射、AI retry |
| `app/src-tauri/src/worker_command.rs` | 1,351 | 命令构造、环境策略、stdin、spawn、监督、取消、平台信号、输出解析、日志摘要 |
| `app/src-tauri/src/transcript_detail.rs` | 1,146 | transcript 读取/保存、路径验证、备份、segments、音频回放缓存 |
| `worker/frameq_worker/bilibili_fallback.py` | 936 | URL 解析、HTTP、API 解析、流选择、下载、DASH 合并、错误映射 |
| `app/src-tauri/src/task_manifest.rs` | 900 | manifest DTO、source identity 校验、artifact 路径和存储安全 |
| `worker/frameq_worker/xiaohongshu_fallback.py` | 894 | URL 解析、HTTP、页面解码、状态解析、流选择和下载 |
| `worker/frameq_worker/pipeline.py` | 875 | 主 pipeline、阶段函数、进度、媒体选择、字幕、ASR、AI helper、路径解析 |
...
flowchart LR
  subgraph DesktopProcess["Desktop process"]
    ...
    subgraph TauriLayer["Tauri / Rust"]
      ...
      Supervisor["worker_command.rs<br/>ProcessSupervisor"]
  ...
  Supervisor -->|"fixed argv + bounded stdin"| CLI
  CLI --> WorkerService
  CLI --> SourceAdapters
  ...
  RustStorage --> Config
  Runtime --> Logs
  CLI -.->|"validated progress over stderr"| Commands
  Commands -.->|"Tauri progress event"| Clients
  Commands -->|"account and entitlement HTTP"| Fastify
  Commands -.->|"session + checkout URL in child env"| InsightFlow
...
- `video_processing.rs` 直接组合 cache、identity preflight、进程监督、日志、进度和结果映射。
...
classDiagram
  ...
  TSProtocol --> RustVideoDTO : Tauri invoke JSON
...
- `ProcessVideoRequest`、`RetryInsightsRequest` 和 output-language 枚举。
...
**怎么读：** 从 `LibRegistry` 看 command 如何注册，再沿 `VideoProcessingModule -> WorkerInvocation -> WorkerCommandSpec` 看子进程如何被描述；`ProcessSupervisor` 一组展示的是独立的生命周期状态机，`TaskManifestModule` 一组展示的是本地存储信任边界。
...
classDiagram
  ...
  class ProcessSupervisors {
    +video: ProcessSupervisor
    +asrModelDownload: ProcessSupervisor
  }
...
| 进程基础设施 | `worker_command.rs`, `ProcessSupervisor` | 构造 command、传 stdin、读取 stdout/stderr、处理取消竞争和平台信号 |
| 本地信任与存储 | `runtime.rs`, `task_manifest.rs`, History/Transcript/Settings modules | 决定哪些目录和文件可读写，拒绝链接、越界路径和不受支持 manifest |

目前这三种责任在若干大文件中交叉出现，所以“拆文件”不能只按函数数量切割。更安全的方式是先确认每个提取模块属于哪种责任，并确保其他代码不能绕过 storage trust boundary 或 ProcessSupervisor。

### `ProcessSupervisor` 到底是什么

它不是一个后台任务队列，也不保存业务结果。它只记录某个 lane 当前受控的 child instance、PID/process group 和 `Running/Cancelling` 阶段，用 instance ID 防止旧 waiter 清除新进程。video 和 model download 各自拥有一个实例，因此两个 lane 可以独立监督，但同一 lane 不允许同时启动两个 child。

### Rust 所有权说明

- `ProcessSupervisor` 是 video worker lane 与 model-download lane 共享的状态机实现，但两个 lane 使用不同实例。
- `video_processing.rs` 既是 IPC adapter，也是 video lane application orchestrator。
- `worker_command.rs` 同时包含领域状态机、平台进程控制和命令基础设施。
- `task_manifest.rs` 是 History、cache、transcript、delete 和 retry 共用的严格存储信任边界。
...
**怎么读：** 从 `CLI -> WorkerService -> Pipeline` 看主调用链，再分别沿 Media、Transcriber、InsightFlow 和 TaskStore 四个方向展开。Protocol 的虚线实现关系表示已有可替换边界。
...
classDiagram
  ...
  TaskStore --> TaskContext
  TaskStore --> ProcessResult
```
...
6. `TaskStore` 负责正式任务目录和 manifest；下载用的临时数据则进入 `cache/tasks/<task_id>`。
...
sequenceDiagram
  ...
  alt Direct request cache hit
    ...
    Tauri->>Supervisor: spawn ResolveSourceIdentity invocation
    Supervisor->>Worker: fixed mode arg + bounded stdin JSON
    Worker->>Worker: resolve_source_identity_once()
    Worker-->>Tauri: canonical SourceIdentity JSON
    ...
    alt Identity cache hit
      ...
      Tauri->>Supervisor: spawn ProcessVideo invocation
      Supervisor->>Worker: --request-stdin + request JSON
      ...
      loop Structured progress lines
        Worker-->>Tauri: FRAMEQ_PROGRESS on stderr
        Tauri->>Tauri: validate against desktop contract
        Tauri-->>Client: Tauri worker-progress event
        Client->>Task: mergeProgressEvent if operationId is current
      end

      Pipeline-->>Worker: ProcessResult
      Worker-->>Tauri: one JSON result on stdout
      Tauri->>Supervisor: finish matching instanceId
      Tauri-->>Client: validated or structured fallback result
    end
  end
...
2. **Rust 做便宜的直接缓存检查。** 如果输入本身已能匹配安全 manifest，就无需启动 Python。
3. **Rust 做 source identity 预检。** 短链接或分享文本可能需要 Python 平台 parser 才能得到稳定 ID；得到 identity 后再做一次严格缓存检查。
4. **Rust 启动正式 worker。** 请求放在 bounded stdin，而不是 argv/environment，避免 URL 出现在命令行和普通诊断信息里。
...
2. `cancel_process` 向 video lane 的 `ProcessSupervisor` claim cancellation。
...
sequenceDiagram
  ...
  Tauri->>Account: server_managed_llm_invocation()
  Account-->>Tauri: checkout URL, session token and request seed
  Tauri->>Supervisor: spawn RetryInsights with bounded stdin and managed env
  Supervisor->>Worker: --retry-insights-stdin
  ...
  Worker-->>Tauri: completed or partial_completed result
  Tauri->>Supervisor: finish matching instanceId
  Tauri-->>Client: WorkerResult
  Client-->>Task: result
...
2. **契约冗余。** 例如未被 pipeline 使用的 request 字段、CLI 兼容重导出。先查调用方和 contract test，再决定保留、恢复语义或版本化删除。
3. **模块职责拆分。** 例如 `server.ts` route、Python platform parser/helper。需要 focused tests，但通常不改变进程模型。
4. **核心所有权和安全边界。** 例如 workflow owner、ProcessSupervisor、task manifest/path validation。必须先写设计与迁移计划，不能用“文件太长”作为唯一理由。
...
| `task_manifest.rs` | DTO、canonical URL 安全、路径安全、artifact IO 都在同一模块 | 如何拆分而不让调用方绕过 strict current-task predicate？ |
| `transcript_detail.rs` | command、路径校验、audio cache、segment IO、备份和 markdown formatting 共存 | 哪些可以成为纯 storage/service 单元；如何避免复制路径安全逻辑？ |
| `worker_service.py` + `pipeline.py` | 两处共同承担 orchestration；source resolver port 贯穿两层，retry 复用 pipeline 中的 AI step | application service、注入 port 与 stage library 的理想边界在哪里？ |
...
| 跨语言 DTO | TS/Rust/Python 手写镜像，contract tests 只覆盖约束的一部分 | 哪些 schema 值得 codegen；哪些应保持语言本地并通过 fixture/contract test 对齐？ |
...
| Worker command and supervision | `app/src-tauri/src/worker_command.rs` |
...
| Task manifest trust boundary | `app/src-tauri/src/task_manifest.rs` |
...
| Task paths and persistence | `worker/frameq_worker/task_store.py` |
...
ProcessSupervisor cancellation race、桌面-worker contract、server Store 事务和本地优先隐私边界。
...
2. 重新核对 `App.tsx` controller wiring、`lib.rs` command registry、Python imports 和 `server.ts` service wiring。
