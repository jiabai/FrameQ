...
| `store.ts` / `prismaStore.ts` | 当前分别为 1,054 / 1,117 个物理行；broad Store port、records、MemoryStore、Prisma CRUD、八类语义事务与并发/retry policy 仍物理集中。方案 2R 设计已获批准，但实现尚未开始 | 设计锁定稳定 root + 私有 transaction slices + consumer-owned narrow ports；必须保留一个 Store 实例、完整语义事务、Memory/Prisma parity、独立客户端并发门禁及现有 schema/migration。见 `docs/design-docs/2026-07-23-server-store-prisma-module-split.md` |
