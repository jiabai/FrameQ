...
该提交是本轮审计的起始基线；本文同时纳入 2026-07-18 与本次实现同批交付的 task-access
facade 重构。后续提交该批改动时，应把上表基准提交更新为实际提交并重新核对规模快照。
...
| 文件 | 物理行数 | 当前可见职责 |
|------|---------:|--------------|
| `app/src-tauri/src/task_manifest.rs` | 1,328 | 私有 manifest DTO/路径原语、`SupportedTask` facade、受限 edit session、source identity 与 artifact 安全 |
| `app/src-tauri/src/video_processing.rs` | 1,266 | IPC/worker DTO、配置解析、缓存查找、source identity 预检、lane 调用、结果映射、AI retry |
