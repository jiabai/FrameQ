...
| Web 宣传站 release metadata 手动同步 | 下载页版本号、平台下载链接、release notes 摘要硬编码在 `site/src/consts.ts` 与 `site/src/content/release-notes/`，新版本发布需手动更新并重新构建。 | `site/src/consts.ts`; `site/src/content/release-notes/`; `docs/exec-plans/completed/2026-08-05-web-marketing-site-plan.md` | 引入 CI 从 GitHub Releases API 注入版本号与下载链接，release notes 从 `docs/releases/` 自动同步到 content collection。 |
