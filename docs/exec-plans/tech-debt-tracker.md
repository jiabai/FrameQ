...
| macOS process-group cancellation needs native-host confirmation | Rust includes a `cfg(unix)` parent-plus-child fixture and the supported macOS runtime uses an isolated PGID with TERM→KILL escalation. In run `29108030194` at `08032c9`, the real macOS fixture passed, but the complete Cargo job failed because six unrelated tests used Windows-only fixture paths. Linux is not a supported release target and is not part of this gate. | `app/src-tauri/src/worker_command.rs`; `.github/workflows/unix-process-supervisor.yml`; `scripts/tests/unix-process-supervisor-workflow.test.mjs`; `docs/product-specs/2026-07-10-desktop-process-supervision-cancellation.md` | Keep the read-only `macos-latest` workflow and close this item only after a run completes the full `cargo test --manifest-path app/src-tauri/Cargo.toml` successfully, with run URL, run ID, commit SHA, and job result recorded. |
