...
| `app-input.browser.test.ts` is intermittently flaky on a timing-sensitive assertion | The locale-freeze test waits on `window.__FRAMEQ_UI_SMOKE__.pending.save_default_generation_preferences?.length === 1`, and under CPU contention the React update does not land inside the wait budget. During the v0.3.7 release gate the same file content produced both `1 failed / 33 passed` and `34 passed` on separate runs, with the activation copy reverted and applied, so the flake is content-independent. It cost release-gate reruns and could either mask a real regression or falsely block a release. | `app/tests/app-input.browser.test.ts` | Replace the polling wait with a signal that does not depend on polling latency, or raise and justify the budget, then demonstrate repeated consecutive green runs of the file. |
| YouTube platform rejections surface as a generic failure with misleading copy | `403` and `429` responses carry none of the markers `classify_youtube_download_failure` recognises, so both fall into the `YOUTUBE_DOWNLOAD_FAILED` bucket whose user-facing copy says to check the network. The v0.3.8 investigation hit exactly this on a perfectly healthy connection, which means the reported symptom actively sends users to debug the one thing that is not broken. | `worker/frameq_worker/media.py` (`classify_youtube_download_failure`); `app/src/workerErrorCopy.ts` | Add `YOUTUBE_STREAM_FORBIDDEN` and `YOUTUBE_RATE_LIMITED` reason codes across the worker registry, the desktop-worker contract, the TypeScript protocol, and three locales, each with copy that names the real cause. |
| Nothing signals that the pinned `yt-dlp` has gone stale | `yt-dlp` is pinned in `uv.lock` while `pyproject.toml` only states a lower bound, and the installed runtime resolves through the lock, so a platform-side extractor change silently turns into a user-visible download failure with no advance warning anywhere in the release pipeline. v0.3.8 fixed one instance of this by hand after a user reported it; the next one will be found the same way unless something watches for it. | `pyproject.toml`; `uv.lock`; `docs/exec-plans/completed/2026-09-19-v0.3.8-desktop-release-plan.md` | Add a scheduled check that compares the locked `yt-dlp` against the latest release and surfaces the drift, or a documented routine that upgrades the pin and reruns a real YouTube download on a fixed cadence. |
...
| Topic | Why it matters | Source | Removal Condition |
|------|----------------|--------|-------------------|
| App-local generation preferences are not key-exact | TypeScript `validateGenerationPreferences` validates the six known values but does not reject additional own keys, and the Rust app-local persistence DTO lacks `deny_unknown_fields`; a valid six-step object with an extra property is therefore accepted and later serialized without that property. This state never crosses the strict worker snapshot and does not reintroduce Profile style/avoid ownership, but tightening released v1/v2 file handling could change recovery or migration outcomes. | `app/src/insightPreferences.ts`; `app/src-tauri/src/insight_preferences.rs`; completed Inspiration Profile v2 boundary ExecPlan | Add TypeScript exact-key acceptance/rejection tests and Rust v1/v2 extra-key migration/recovery tests, define whether the affected default alone is cleared or the file is reset, and prove an extra generation-preference key cannot cause collateral reset of an otherwise valid profile before tightening both boundaries together. |
| Preference label IDs permit whitespace-only JSON-schema strings | Runtime TypeScript/Rust/Python boundaries close label field identities and require label values to be strings, but the shared JSON schema currently expresses label IDs with `minLength: 1` rather than a non-whitespace pattern. Current option IDs are fixed safe registries and producer tests prevent a practical malformed ID, so this does not weaken the six-field profile or seed boundary; it is a schema precision mismatch. | `contracts/desktop-worker-contract.json`; `app/src-tauri/src/video_processing/retry_insights.rs`; `worker/frameq_worker/requests.py`; completed Inspiration Profile v2 boundary ExecPlan | Tighten the shared label-ID schema and all three runtime validators together, with cross-language whitespace-only RED tests, when the label snapshot contract next changes. |
...
| Web 宣传站 release metadata 手动同步 | 下载页版本号、平台下载链接、release notes 摘要硬编码在 `site/src/consts.ts` 与 `site/src/content/release-notes/`，新版本发布需手动更新并重新构建。宣传站为人工部署流程，不引入自动化。 | `site/src/consts.ts`; `site/src/content/release-notes/`; `docs/exec-plans/completed/2026-08-05-web-marketing-site-plan.md` | 维护人工更新流程：发布新版本时同步更新 `consts.ts` 版本号与下载链接，从 `docs/releases/` 复制 release notes 摘要到 content collection，重新构建并人工部署 `site/dist/`。 |
| Web 宣传站多语言缺失 | 首版仅中文。多语言切换需先建立翻译审阅流程（对齐 `docs/DESIGN.md` 简繁中文审阅规则）。 | `site/src/pages/*.astro`; `docs/exec-plans/completed/2026-08-05-web-marketing-site-plan.md` | 建立翻译审阅流程后，在 Astro 中引入 i18n routing（`src/pages/[lang]/`）与语言切换组件。 |
...
| Web 宣传站博客/changelog 自动同步缺失 | release notes 通过 content collections 手动同步，无自动拉取。博客功能未实现。宣传站为人工部署流程。 | `site/src/content.config.ts`; `site/src/content/release-notes/` | 维护人工同步流程：发布新版本时从 `docs/releases/` 复制 release notes 摘要到 content collection；如需博客功能，按需引入博客 content collection 与 RSS。 |
| Web 宣传站真实浏览器视觉验证未跑 | 自动化测试断言 HTML 结构与 token 使用，移动端用 browser subagent（Chromium 内核）验证。Firefox/Safari 跨内核视觉一致性、Lighthouse CLI 性能/SEO 审计未跑。V2 动效（视差/3D 倾斜/涟漪/鼠标跟随）在 Firefox/Safari 下的表现同样未实测。 | `docs/exec-plans/completed/2026-08-05-web-marketing-site-plan.md` Task 6; `docs/exec-plans/completed/2026-08-06-web-site-dynamic-enhancement-plan.md` Task 7 | 在发布前手动跑一次跨浏览器视觉回归（Firefox/Safari）与 Lighthouse CLI 审计，覆盖 V1 静态布局与 V2 动效两层；或在本地开发环境跑 Lighthouse。 |
...
|------|----------|--------|
| Web 宣传站部署 CI 未定 | 用户确认宣传站为人工部署流程，不引入 CI 自动化。`site/dist/` 产物由人工托管到任意静态站点服务；`astro.config.mjs` 的 `site` 字段在确认真实域名后人工更新。相关技术债（release metadata 手动同步、博客/changelog 自动同步、真实浏览器视觉验证）的 Removal Condition 已同步调整为人工流程。 | 2026-08-06 |
