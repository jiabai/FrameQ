...
- 2026-08-23 fresh packaged Windows smoke (v0.3.5 installer, `FrameQ_0.3.5_x64-setup.exe`): installer resource tree contains app-local `msvcp140.dll` (643,512 bytes) and `vcruntime140_1.dll` (50,112 bytes) beside bundled Python; packaged `python.exe` imports numpy 1.26.4 / onnxruntime 1.28.0 / funasr_onnx / sklearn 1.9.0 successfully; `frameq_worker.cli` and `worker_service` facade (incl. `run_asr_model_download_once`) import cleanly; app launched and stayed alive; logs recorded `worker.download_asr_model.exit pid=30432 exit=0 outcome=structured` and a following `process_video` exit=0, confirming the packaged worker starts and the model-download path runs without runtime-missing failure. Installer Authenticode is `NotSigned` (known status, out of scope).

## Residual Risk

- ~~A previously installed package built before the app-local runtime fix may still lack DLLs; it must be rebuilt/reinstalled before retesting.~~ Closed by the 2026-08-23 fresh v0.3.5 packaged smoke (DLLs present in installer resource tree).
- ~~A fresh packaged Windows smoke remains necessary to verify the actual installer resource tree and Python extension loading. Existing installations built before this fix must be rebuilt/reinstalled. The local bundled-Python import smoke now passes after dev staging.~~ Closed on 2026-08-23: v0.3.5 installer verified on this host — app-local VC++ DLLs present, packaged Python extensions and worker facade import cleanly, app launched, and logs show a successful `download_asr_model` worker run (exit=0).
