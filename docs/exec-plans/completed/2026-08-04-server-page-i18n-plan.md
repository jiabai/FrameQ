...
**Architecture:** A new `server/src/i18n.ts` module owns the `Locale` type, the flat-keyed string dictionary for all three locales, the `t()` lookup, the `detectLocale()` resolver, the `renderLangSwitcher()` button HTML/inline-script, and the shared `langSwitcherStyles()` CSS. The four page-render functions (`renderLoginPage`, `renderDashboardPage`, `renderAdminLoginPage`, `renderAdminPage`) gain a `locale` parameter, replace hard-coded literals with `t(locale, key)`, emit `<html lang="...">`, render the switcher button, and inject a `buildClientStrings(locale)` JSON blob for client-side script use. The four page routes (`GET /login`, `GET /dashboard`, `GET /admin/login`, `GET /admin`) read the cookie, deep-link, and `Accept-Language` signals and forward the resolved locale to the render function. No new routes, no new store methods, no new Prisma models, no new env vars, no new design doc.
...
- Out of scope for the initial v1 implementation: desktop binary; worker; ASR model path;
  OTP/auth/store/Prisma behavior; new design doc; version number. The initially deferred
  `zh-TW` server locale and `Accept-Language` fallback were subsequently implemented on `main`.
...
- [x] 2026-08-05: Archived. All three tasks complete; i18n-specific tests (29 unit + 12 integration) landed.
  Plan moved to `completed/`. The v0.3.1 tagged release initially shipped `zh-CN` and `en`; the
  subsequently landed Server i18n follow-up on `main` adds reviewed `zh-TW` copy and
  `Accept-Language` fallback. Remaining risks are the non-`Secure` preference cookie and the
  unrecorded real-browser click-through.

## Decision Log

- Decision (superseded by the post-release follow-up): Limit server-side locales to `zh-CN` (default)
  and `en` for v1, and detect locale only from the `lang` cookie. The initial release intentionally
  deferred reviewed `zh-TW` copy and `Accept-Language`; current `main` supports all three locales
  and resolves `lang` cookie → `?lang=` deep link → `Accept-Language` → default. Date/Author:
  2026-08-04, Codex; superseded 2026-08-07.
...
- **`zh-TW` server-side locale — resolved on current `main`.** The server supports reviewed
  Traditional Chinese copy without runtime conversion.
- **`Accept-Language` fallback — resolved on current `main`.** First visits use the browser header
  after the `lang` cookie and `?lang=` deep link; explicit user choices still win.
...
- The initially deferred `zh-TW` server-side locale and `Accept-Language` fallback are resolved on
  current `main`; see the post-release i18n follow-up and `server/src/i18n.ts`.
